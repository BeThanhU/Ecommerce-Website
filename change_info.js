function change_user_info(){
	...
	var method = "GET";
	var url = "./change_info.php?change=userChangeInfo&userName="+user_Name+"&userCompany="+user_Company+"&userFAX="+user_FAX;
...
function change_product_info(){
	...
	var ajax = new XMLHttpRequest();
	var method = "GET";
	var url = "./change_info.php?change=productChangeInfo&productID="+product_ID+"&productName="+product_name+"&productPacking="+product_packing+"&productPrice="+product_price+"&productAmount="+product_amount+"&productCategoryID="+product_categoryID+"&productSaleoff="+product_saleoff+"&productCategory="+product_category+"&productManufacturer="+product_manufacturer+"&productCountry="+product_country+"&productStatus="+product_status+"&productWarranty="+product_warranty+"&productSpecial="+product_special;
	var asynchronous = true;
	...
	ajax.onreadystatechange = function(){
		if(this.readyState == 4 && this.status == 200){
			...
			if(response == "true"){
				alert('Thay đổi thông tin thành công!');
				window.location = "./trangquantri.php?quanly=hanghoa";
			}else{
...
function change_bill_status(ID, staffID){
	...
	var ajax = new XMLHttpRequest();
	var method = "GET";
	var url = "./change_info.php?change=billChangeStatus&staffID="+staff_ID+"&billID="+bill_ID+"&billStatus="+bill_ID_Status;
	var asynchronous = true;
	...
	ajax.onreadystatechange = function(){
		if(this.readyState == 4 && this.status == 200){
			...
			if(response == "true"){
				...
				window.location = "./trangquantri.php?quanly=donhang";
			}else{
...
function remove_user_address(num){
	...
	var ajax = new XMLHttpRequest();
	var method = "GET";
	var url = "./change_info.php?change=userRemoveAddress&userAddressNumber="+address_number;
	var asynchronous = true;
...
function remove_item_cart(num){
	...
	var ajax = new XMLHttpRequest();
	var method = "GET";
	var url = "./change_info.php?change=userRemoveCartItem&userName="+user_name+"&productID="+product_ID;
	var asynchronous = true;
...
function add_user_address(id){
	...
	if(user_Address){
		var ajax = new XMLHttpRequest();
		var method = "GET";
		var url = "./change_info.php?change=userAddAddress&userID="+user_ID+"&userAddress="+user_Address;
		var asynchronous = true;
...
function reset_user_password(userName){
	...
	var ajax = new XMLHttpRequest();
	var method = "GET";
	var url = "./change_info.php?change=userResetPass&userName="+user_Name;
	var asynchronous = true;
	...
	ajax.onreadystatechange = function(){
		if(this.readyState == 4 && this.status == 200){
			...
			if(response != "false"){
				...
				window.location = "./trangquantri.php?quanly=khachhang";
			}else{
				alert('Không thể xóa địa chỉ, vui lòng liên hệ ban quản trị để được xử lý');
			}
		}
...
function reset_staff_password(staffName){
	...
	var ajax = new XMLHttpRequest();
	var method = "GET";
	var url = "./change_info.php?change=staffResetPass&staffName="+staff_Name;
	var asynchronous = true;
	...
	ajax.onreadystatechange = function(){
		if(this.readyState == 4 && this.status == 200){
			...
			if(response != "false"){
				alert('Reset pass thành công, mật khẩu mới của bạn là: ' + response);
				window.location = "./trangquantri.php?quanly=taikhoan";
			}else{
				alert('Không thể xóa địa chỉ, vui lòng liên hệ ban quản trị để được xử lý');
			}
		}
